...
App = {
  ...
  render: function () {
    ...
    for (var id = 0; id < App.policies.length; id++) {
      ...
      var header = "<div class=\"modal-header text-center\"><h2 class=\"modal-title w-100\">" + name + "</h2></div>"
      ...
      var button = "<button type=\"submit\" class=\"btn btn-outline-primary\">" + "Vote" + "</button >"
      ...
      var body = "<div class=\"modal-body\"><p> Description: " + description + "</p> Time Left: " + timer + table + form + "</div>"
  ...
  castVote: function (id) {
    ...
    contract.vote(option_id, function (error, result) {
      if (!error) {App.listenForNewVote(address);}
      else { console.log(error) }
    });
  },
  ...
  countdown: function (id, policy_box) {
    var address = App.policies[id][0]
    var proposal_creation = new Date(App.policies[id][3]);
